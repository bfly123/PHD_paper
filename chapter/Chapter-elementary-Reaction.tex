...
上一章我们探讨了在固定比热比的总包反应模型和多步反应模型中的空间刚性问题。如果计算需要更准确的模拟化学反应动力学过程时，一般采用基元反应模型进行模拟。在应用了基元反应模型的化学反应的计算中，通常不会出现空间刚性导致的非物理解，但是问题并没有因此变得简单，基元反应模型求解中会遇到更多的难点：
\begin{enumerate}[1.]
  \item 首先不同组分间的空间尺度相差很大，如果网格不足，空间尺度较小的组分就难以准确计算，想要达到完全求解具有最小空间尺度的组分是极为困难的。关于尺度问题见附录 \ref{chap:scale}。
  \item 采用差分格式计算组分对流过程时，由于格式振荡和源项的作用，并不能保证组分的守恒。
  \item 基元反应模型中的源项ODE方程时间刚性问题比上一章采用简单反应模型时更为严重。
\end{enumerate}

考虑到无论是固定比热比的多步反应模型，还是基元反应模型，方程并没有本质变化，只是源项形式有所区别，而遇到的空间问题归根结底也都是由于网格不足和方法中的数值耗散造成的。因此，本章将针对带有基元反应模型的爆轰求解，研究DIP方法在基元反应模型中的有效性和鲁棒性。
...
\begin{equation}
  ...
  \end{equation}
  求得，具体参数见表 \ref{tab:HO}。
\input{chapter/HO_table.tex}
\subsection{数值结果与分析}
本小节中分别计算四种不同工况下的氢氧爆轰一维算例，并将计算结果和理论值及实验值进行对比。计算域长为$L$，左边采用固壁条件，右侧采用出流条件。最开始整个区域充满了H$_2$，O$_2$ 和Ar混合物，在固壁端设置有一小段高温、高压点火区域，短时间内迅速引爆混合气体，产生右行的爆轰波。点火参数为$P_\text{ign}=28.0P_0$、$\rho_\text{ign}=5.0\rho_0$ 和 $T_\text{ign}=13.0T_0$。
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.5\textwidth}
				  ...
				  \includegraphics[width=6cm]{HO2.png}
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.5\textwidth}
				  ...
				  \includegraphics[width=6cm]{H2O2Ar.png}
					\centerline{a:压力分布随时间变化图}
 \end{minipage}
 \hfill
 \begin{minipage}[t]{0.5\textwidth}
			...
			%	  \label{fig:side:a}
				  ...
				  \includegraphics[width=6cm]{H2O2Ar_2.png}
				  \centerline{b:压力分布放大图}
				\end{minipage}
\vfill
 \begin{minipage}[t]{0.5\textwidth}
				  %\includegraphics[width=6cm]{speed_H2O2Ar.eps}
				  \includegraphics[width=6cm]{speed_H2O2Ar.png}
				  \centerline{c:波速随时间变化图}
				  \end{minipage}
\hfill
 \begin{minipage}[t]{0.5\textwidth}
				  %\includegraphics[width=6cm]{H2O2Ar_T.eps}
				  \includegraphics[width=6cm]{H2O2Ar_T.png}
				  \centerline{d:温度分布随时间变化图}
				\end{minipage}
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.5\textwidth}
				  ...
				  \includegraphics[width=6cm]{H2O27Ar_P.png}
					\centerline{a:压力分布随时间变化图}
 \end{minipage}
 \hfill
 \begin{minipage}[t]{0.5\textwidth}
			...
			%	  \label{fig:side:a}
				  ...
				  \includegraphics[width=6cm]{H2O27Ar_Z.png}
				  \centerline{b: $T=1,800\mu s$时刻的组分分布}
				\end{minipage}
\vfill
 \begin{minipage}[t]{0.5\textwidth}
				  %\includegraphics[width=6cm]{H2O27Ar_speed.eps}
				  \includegraphics[width=6cm]{H2O27Ar_speed.png}
				  \centerline{c:波速随时间变化图}
				  \end{minipage}
\hfill
 \begin{minipage}[t]{0.5\textwidth}
				  %\includegraphics[width=6cm]{H2O27Ar_T.eps}
				  \includegraphics[width=6cm]{H2O27Ar_T.png}
				  \centerline{d:温度分布随时间变化图}
				\end{minipage}
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.5\textwidth}
				  ...
				  \includegraphics[width=6cm]{H2O21033Ar_P.png}
			\centerline{a:压力分布随时间变化图}
 \end{minipage}
 \hfill
 \begin{minipage}[t]{0.5\textwidth}
			...
			%	  \label{fig:side:a}
				  ...
				  \includegraphics[width=6cm]{H2O21033Ar_speed.png}
				  \centerline{b: $T=1,800\mu s$时刻的组分分布}
				\end{minipage}
\vfill
 \begin{minipage}[t]{0.5\textwidth}
				  %\includegraphics[width=6cm]{H2O21033Ar_T.eps}
				  \includegraphics[width=6cm]{H2O21033Ar_T.png}
				  \centerline{c:波速随时间变化图}
				  \end{minipage}
\hfill
 \begin{minipage}[t]{0.5\textwidth}
				  %\includegraphics[width=6cm]{H2O21033Ar_Z.eps}
				  \includegraphics[width=6cm]{H2O21033Ar_Z.png}
				  \centerline{d:温度分布随时间变化图}
				\end{minipage}
...
\normalsize
\subsection{组分守恒性的比较}
在该小节中我们选上小节中第四个算例，采用相同的网格$N=12000$和计算时间$T=1200\mu s$，对本文发展的方法和直接WENO方法进行比较。
...
\begin{figure}
  ...
  \begin{minipage}[t]{0.5\textwidth}
				  ...
				  \includegraphics[width=6cm]{compare_WENO_P.png}
					\centerline{a:压力分布对比}
%				  \label{fig:compare:a}
 \end{minipage}
 \hfill
\begin{minipage}[t]{0.5\textwidth}
			...
			%	  \label{fig:side:a}
				  ...
				  \includegraphics[width=6cm]{compare_speed.png}
				  \centerline{b:波速变化对比} 
%				  \label{fig:4:b}
				\end{minipage}
\vfill
 \begin{minipage}[t]{0.5\textwidth}
			...
			%	  \label{fig:side:a}
				  ...
				  \includegraphics[width=6cm]{compare_WENO_O2.png}
...
%				  \label{fig:4:b}
 ...
 \hfill
  \begin{minipage}[t]{0.5\textwidth}
				  ...
				  \includegraphics[width=6cm]{compare_WENO_Ar.png}
...
%
%				  \centerline{b:H$_2$O组分分布对比} 
%%				  \label{fig:4:b}
%				\end{minipage}
%\vfill
%  \begin{minipage}[t]{0.5\textwidth}
%				  \includegraphics[width=6cm]{compare_WENO_O2_2.eps}
%					\centerline{c:O$_2$组分分布放大对比}
%%				  \label{fig:compare:a}
% \end{minipage}
% \hfill
% \begin{minipage}[t]{0.5\textwidth}
...
%
\subsection{计算精度的比较}
由于通常计算中，尤其在高维度问题的计算中，能采用的网格步长会远大于上面计算用到的网格步长，因此在这里我们验证在稀疏网格下新方法的效果。

首先我们验证上面的计算结果是否达到网格收敛，以便选用合理的参考解。
这里采用$N=6000$网格量和$N=12000$网格量进行对比，图 \ref{fig_grid_convergence} 给出两种网格下的计算结果，可以看到在$N=6000$网格下，计算结果已经收敛。

			  \begin{figure}
  %\centering
  \begin{minipage}[t]{0.5\textwidth}
				  %\includegraphics[width=7cm]{grid_convergence_P.eps}
				  \includegraphics[width=7cm]{grid_convergence_P.png}
					\centerline{a:压力分布}
%				  \label{fig:compare:a}
 \end{minipage}
 \hfill
 \begin{minipage}[t]{0.5\textwidth}
			...
			%	  \label{fig:side:a}
				  ...
				  \includegraphics[width=7cm]{grid_convergence_D.png}
				  \centerline{b:密度分布} 
%				  \label{fig:4:b}
				\end{minipage}
\vfill
  \begin{minipage}[t]{0.5\textwidth}
				  ...
				  \includegraphics[width=7cm]{grid_convergence_T.png}
					\centerline{c:温度分布}
%				  \label{fig:compare:a}
 \end{minipage}
 \hfill
 \begin{minipage}[t]{0.5\textwidth}
			...
			%	  \label{fig:side:a}
				  ...
				  \includegraphics[width=7cm]{grid_convergence_O2.png}
				  \centerline{d:$O_2$组分分布} 
%				  \label{fig:4:b}
				\end{minipage}
  \caption{不同网格下计算结果对比}
...
下面我们在网格$N=600$下，对比新方法和传统方法的计算结果，图 \ref{fig_grid_600} 分别给出压力、温度等物理量的对比，可以看到，新方法比传统方法具有好的精度效果。
...
%				  \label{fig:4:b}
				\end{minipage}
  \caption{稀疏网格下不同方法计算结果对比，$N=600$}
  \label{fig_grid_600}
\end{figure}

\subsection{二维爆轰}
在本章的最后我们计算二维维爆轰算例，用以验证新方法在化学反应计算中的可靠性。
