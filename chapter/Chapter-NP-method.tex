...
如前言中所讨论的，目前除隐式格式外，也有很少的一些显式格式在一定程度满足刚性稳定的要求，例如，线化梯形公式（LT）\cite{wang2012high,zhang2014equilibrium}，该方法具有二阶精度、显式形式和一定程度的刚性稳定性，因而在如化学反应这种同时存在刚性和非刚性区域，又由于特征矩阵巨大而难以采用隐式格式求解的方程中得到较多的应用。然而，线化梯形公式也仅仅具有二阶精度，在非刚性的区域精度不足，另外，该方法也非绝对的刚性稳定的，刚性过大仍然会出现计算问题。
...
\matrix[left =1cm of big,column sep=0mm]
	...
	\end{tikzpicture}
	\caption{不同格式求解ODE方程  (\ref{eqexample1}) 结果}
\label{fig:unlinear1}
\end{figure}
