...
\end{equation}
其中 $\bm{U}=(\rho, \rho u,\rho E)^T$，$\bm{F}=\left( \rho u,\rho u^2+p,u(E+p) \right)^T$
...
\begin{equation}
  ...
  \begin{array}{l}
	...
	\right)
\end{equation}
通量$\bm{F}^C$特征值为$(u,u,u)$，代表对流过程，通量$\bm{F}^P$具有特征值$(0,-a,a)$，代表各个方向延声速传播的压力波。根据它们的不同特征，Zha et al.\cite{zha1993numerical,zha1999numerical,zhacomparative}建议对于对流项采取迎风重构，对于压力项在亚声速区采用同时考虑上游和下游的重构方式。

对于磁流体特征系统，特征值中含有三个不同的特征波速---快波$c_f$、慢波$c_s$和中速Alfv\'{e}n波，异常复杂，目前尚没有和流体力学方程$\bm{F}^P$项对应的精确的通量公式。然而由于磁流体方程有和流体力学方程相似的波动特性，E-CUSP的基本思想也可以推广到求解MHD方程中来。
...
数值模拟MHD方程时，需要保证磁场散度为零的限制条件，$\Delta \cdot \bm{B}=0$。有几种方法可以满足这一条件，Powell等人通过在原方程中添加一个和$\Delta \cdot \bm{B}$成正比的源项，得到一个新的特征系统。但是，这一系统会存在一些不确定性\cite{powell1999solution}。投影方法\cite{brackbill1980effect}通过求解泊松方程和限制边界条件来满足磁场散度为零的条件。Balsara和Spicer\cite{balsara1999staggered}提出的控制输运方法是另外一种可以将散度误差控制到机器误差精度的方法。Toth\cite{toth2000b}认为控制输运方法是他所有测试过的方法中最准确的。因此本文采用控制输运方法。

首先，给出法拉第方程
\begin{equation}\label{eq:BE}
  \frac{\partial \bm{B}}{\partial t}+\Delta \times \bm{E}=0
\end{equation}
对于理想磁流体电场$\bm{E}$为
\begin{equation}
  \bm{E} =-v\times \bm{B}
...
，磁场强度$\bm{B}$沿区域边缘取平均值$\bm{B}$, 方程\ref{eq:BE} 的半离散形式:
  \begin{equation}
	\begin{array}{l}
	  \frac{d\overline{\bm{B}}_{x_{\!i\!-\!1\!/\!2,j,k}}}{dt}\!=\!-\frac{\bm{E}_{z_{i\!-\!1\!/\!2,j\!+\!1\!/\!2,k}}\!-\!\bm{E}_{z_{i\!-\!1\!/\!2.j\!-\!1\!/\!2,k}}}{\delta y}\!+\!\frac{\bm{E}_{z_{i\!-\!1\!/\!2,j,k\!+\!1\!/\!2}}\!-\!\bm{E}_{z_{i\!-\!1\!/\!2,j,k\!-\!1\!/\!2}}}{\delta z},\\
	    \frac{d\overline{\bm{B}}_{x_{i,j,k\!-\!1/2}}}{dt}\!=\!-\frac{\bm{E}_{z_{i\!+\!1/2,j,k\!-\!1\!/\!2}}\!-\!\bm{E}_{z_{i\!-\!1\!/\!2,j\!-\!1\!/\!2,k}}}{\delta x}\!+\!\frac{\bm{E}_{z_{i\!-\!1\!/\!2,j,k\!+\!1\!/\!2}}\!-\!\bm{E}_{z_{i\!-\!1\!/\!2,j,k\!-\!1\!/\!2}}}{\delta y},\\
	  \frac{d\overline{\bm{B}}_{x_{i,j\!-\!1/2,k}}}{dt}\!=\!-\frac{\bm{E}_{z_{i\!+\!1\!/\!2,j\!-\!1\!/\!2,k}}\!-\!\bm{E}_{z_{i\!-\!1\!/\!2.j\!-\!1\!/\!2,k}}}{\delta x}\!+\!\frac{\bm{E}_{z_{i,j\!-\!1\!/\!2,k\!+\!1\!/\!2}}\!-\!\bm{E}_{z_{i,j\!-\!1\!/\!2,k\!-\!1\!/\!2}}}{\delta z},
	\end{array}
	\end{equation}
	其中，
	\begin{equation*}
	  \begin{array}{l}
		\bm{E}_{x_{i,j+1/2,k+1/2}}\!=\!\frac{1}{4}(\bm{H}_{7i,j,k+1/2}\!+\!\bm{H}_{7i,j+1,k+1/2}\!-\!\bm{G}_{8i,j+1/2,k}\!-\!\bm{G}_{8i,j+1/2,k+1}),\\
		\bm{E}_{y_{i+1/2,j,k+1/2}}\!=\!\frac{1}{4}(\bm{F}_{8i+1/2,j,k}\!+\!\bm{F}_{8i+1/2,j,k+1}\!-\!\bm{H}_{6i,j,k+1/2}-\bm{H}_{6i+1,j,k+1/2}),\\
		\bm{E}_{z_{i+1/2,j+1/2,k}}\!=\frac{1}{4}(\bm{G}_{6i,j+1/2,k}+\bm{G}_{6i+1,j+1/2,k}\!-\!\bm{F}_{7i+1/2,j,k}\!-\!\bm{F}_{7i+1/2,j+1,k}),
		\end{array}
	  \end{equation*}
	  其中$\bm{F}_n$,$\bm{G}_n$,$\bm{H}_n$ ($n=6,7,8$) 是三维空间的第n通量。
