...
化学反应流（Reacting flow）也称为流体热化学（Aerothermochemistry），指的是在流动过程中伴随着化学反应产生的传质传热过程。在化学反应流中，本文主要关注刚性化学反应流和爆轰波两个与激波相关的物理问题。
\subsubsection{刚性化学反应流}
...
一般的化学反应流模拟中，我们都可能会遇到一定的化学反应源项刚性，但是这种刚性只体现为时间尺度问题。时间刚性尽管可以通过算子分裂方法减小化学反应步时间步长或者采用隐式时间方法可以解决这一问题\cite{najm1998semi}，但是想构造高阶的、刚性稳定的时间方法构造仍然有巨大的难度，而高阶隐式格式又由于需要过大的计算量也并非理想的选择。
