...
方程 (\ref{Afrac}) 恰好是界面运动的控制方程，这也不难理解，化学反应本身就是一种界面问题，因此，采用界面方法捕捉或者追踪化学反应界面是一个很直接的想法，难点在于化学反应界面是有厚度的耗散界面，因此很多传统的界面方法都难以应用在该类问题上。
...
本章的计算中，我们采用新的分离方法将方程分裂成三部分，在Euler部分 (\ref{EulerEquation}) 我们采用LF分裂方法 (\ref{eq:LF})、五阶WENO空间离散格式 (章节 \ref{chap:WENO}) 和三阶Runge-Kutta时间离散格式 (\ref{eq:RK3})；在组分对流步 (\ref{Afrac}) 采用DIP方法；在反应步采用3TNP格式。作为对照，我们采用\cite{zhang2014equilibrium,bao2000random,wang2012high}等引文中常用的分裂方法，将方程 (\ref{ReactionEuler}) 算子分裂成如下对流步和反应步两部分
