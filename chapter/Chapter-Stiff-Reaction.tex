...
方程 (\ref{eq:linear_source}) 中的$\mu$是可调参数，如果$\mu$很大时候，ODE方程 (\ref{eq:R_step}) 会存在时间刚性，需要采用稳定性好的ODE求解方法，如NP格式，进行求解。为了便于说明问题，这里采用一种特殊的初始条件
\begin{eqnarray*}
  z_0(x)=\left\{
\begin{array}{lcl}
	1,&  & {x \leqslant 0.3}\\
...
在化学反应流动方程中这种非物理解的产生过程会更为复杂，但是产生原因是类似的，关于这一点，接下来我们将在反应欧拉方程的分析中得到验证。
...
是有厚度的耗散界面，因此我们应该采用针对于耗散界面的处理方法。这里以粒子点为例，给出需要修正的伪代码 (参照章节 \ref{chap:code_1d}），其中$\bm Z =(z_1,z_2,\cdots,z_{ns-1})^T$，其它如$\bar{\bm Z}$、$\bar{\bm Z}_p$是类似的。
