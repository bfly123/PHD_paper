...
\begin{enumerate}[1, ]
	\item
	  界面捕捉类方法由于采用全计算域求解可以更容易计算界面膨胀、压缩，界面张力等光滑函数问题\cite{enright2002hybrid}；界面追踪类方法由于采用拉格朗日运动思想可以更好的保持界面形状。我们能否同时具备两方面的优点呢？
\item 传统界面方法一般并不直接求解方程 （\ref{eq:interface_mark}）中和物理相关的输运量$z$ ，因此较难处理多相界面和耗散界面。界面方法能否针对输运量$z$进行求解？
\end{enumerate}
...
粒子类界面方法主要包括早期的Particle-In-Cell（PIC）方法和Marker-In-Cell（MAC）方法。PIC方法采用携带有质量的真实粒子模拟流体的运动，根据粒子携带的信息可以判断界面位置，然而，由于真实粒子无法人为的生成和抹去，无法保证全计算域每个网格都含有粒子。Marker-and-cell (MAC)方法采用标记粒子，粒子除位置以外不含有其它信息，是完全虚拟的粒子，增加和减少都不会影响流场的物理性质，但是由于它完全不携带信息，只能通过某单元内有粒子和没有粒子来判断单元物质状态和界面位置，因此无法做到覆盖计算域。

在数值模拟中，采用空间网格方法对真实物理场进行离散时，这种离散是真实物理信息在网格点的空间位置上的近似，在计算中，可以加密网格、减少网格或移动网格，尽管这可能影响计算精度，但是并不会改变所研究的物理问题。 类比于网格点，在求解方程 (\ref{eq:interface_mark}) 时，我们引入一套和网格点相似的、携带有输运值的粒子，这种粒子同样可以根据需要增加、减少和移动。
...
对于固定网格方法，网格点的位置是确定的，每个时刻都能得到确定的空间点上的计算值，并可以通过高阶插值运算得到空间任何点的近似值；对于运动网格，如自适应加密网格，也可以人为的控制网格加密规律，避免过分畸形的网格分布和网格形状。然而，粒子点的运动完全是由速度场决定的，所以粒子点的分布是不可控的。MAC方法也遇到相同的困难，某单元原本含有粒子，但是随着流场膨胀，单元不再含有粒子，但这并不代表该单元产生了相变。MAC方法的解决策略是在初始计算时，在相应状态的单元内放置多达16个粒子来避免这种错误的发生。但是这样即增加计算和存储，又不能从根本上避免粒子分布不可控这一问题。

由于我们构造的信息点是可以根据需要生成和抹去的，因此信息点在一定程度上是可控的，因此我们构造了一个限定在单元内且唯一的信息点，该信息点既可以解决粒子分布问题，又可以用来传递单元上和粒子上的输运值。我们将这种信息点称为单元信息点，简称为{\bf{单元点（cell-point）}}。下面我们以一维为例来介绍单元点求解方程 (\ref{eq:interface_mark}) 的步骤。
\begin{enumerate}
  \item[一.] {\bf{初始化}}
...
然而，单元点并不完全是拉格朗日追踪粒子，尽管它弥补了粒子不能全域覆盖、无法计算耗散界面等缺点，但由于每步的更新，新的单元点会不断替代旧的单元点，而新的单元点的生成存在误差，导致对界面结构的保持并不像纯脆的粒子方法，如MAC方法那么好。为了改进这一不足，继续保持粒子方法优良的特性，我们引入了另外一套全域追踪的信息点，在每一步对单元点进行修正。将这种全域追踪、具有纯粒子点特性的信息点，称为{\bf{粒子信息点}}，简称为粒子点。下面我们给出通过粒子点修正单元点的过程。
...
对于粒子点，边界无法穿透，因此如果有粒子穿越边界，将它做反射
...
$i_1=-1,1$， $j_1=-1,1$。权重函数反比于距离，
 ...
 $$L_{i_1,j_1}=\sqrt{(X(i+i_1,j+j_1)+i_1)^2+(Y(i+i_1,j+j_1)+j_1)^2}$$
	...
	二维边界处理和一维是相似的，只是由于粒子点标记不再是一维的，随着入流和出流，粒子点标记变化会复杂一些，下面进行简单说明。假设$i=0$一侧为入流边界，对于粒子点入流边界条件为
...
\begin{figure}
  ...
  \caption{一维组合波的传播，不同计算方法计算结果比较，$T=6$}
