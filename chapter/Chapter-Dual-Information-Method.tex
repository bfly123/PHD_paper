...
\subsection{信息点的概念}
粒子方法是一种最直观的界面方法，具有优良的拉格朗日追踪特性，但是由于它们明显的不足，学者们更愿意将它们作为一种辅助手段，如用作示踪粒子，或者和其它方法结合，用粒子来修正计算结果，如level set-粒子方法\cite{enright2002hybrid,enright2005fast,li2008hybrid}，VOF-粒子方法法\cite{grosshans2014development}等。粒子方法最大的问题是粒子的离散性和分布难以管理，无法保证全域的覆盖，这样就很难像网格方法一样准确的得到任意空间位置的信息。
...
对于固定网格方法，网格点的位置是确定的，每个时刻都能得到确定的空间点上的计算值，并可以通过高阶插值运算得到空间任何点的近似值；对于运动网格，如自适应加密网格，也可以人为的控制网格加密规律，避免过分畸形的网格分布和网格形状。然而，粒子点的运动完全是由速度场决定的，所以粒子点的分布是难以人为控制的。MAC方法也遇到相同的困难，某单元原本含有粒子，但是随着流场变化，单元不再含有粒子，但并不代表该单元一定产生了相变。MAC方法的解决策略是在初始计算时，在相应状态的单元内放置多达16个粒子来避免这种错误的发生。但是这样即增加计算和存储，又不能从根本上避免粒子分布难以控制这一问题。

由于信息点可以根据需要生成和抹去，在一定程度上是可控的。因此，可以构造了一个限定在单元内且唯一的信息点，该信息点既可以解决粒子分布问题，又可以用来传递单元上和粒子上的输运值。我们将这种信息点称为单元信息点，简称为{\bf{单元点（cell-point）}}。下面以一维为例来介绍单元点求解方程 (\ref{eq:interface_mark}) 的步骤。
\begin{enumerate}
  \item[一.] {\bf{初始化}}
...
然而，单元点并不完全是拉格朗日追踪粒子，尽管它弥补了粒子不能全域覆盖、无法计算耗散界面等缺点，但由于每步的更新，新的单元点会不断替代旧的单元点，而新的单元点的生成存在误差，导致对界面结构的保持并不像纯的粒子方法，如MAC方法，那么准确。为了改进这一不足，继续保持粒子方法优良的特性，我们引入了另外一套全域追踪的信息点，在每一步对单元点进行修正。将这种全域追踪、具有纯粒子点特性的信息点，称为{\bf{粒子信息点}}，简称为粒子点。下面我们给出通过粒子点修正单元点的过程。
...
\begin{eqnarray*}
  ...
  \item[三.]{\bf 粒子点修正单元点}
	...
	粒子点修正单元点的过程和通过单元点更新单元点是相似的，只是将单元点换成粒子点。具体过程可以参见附录\ref{chap:2D_PID} 中的过程。
