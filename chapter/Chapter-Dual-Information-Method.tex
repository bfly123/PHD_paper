...
\begin{enumerate}
  \item 均匀速度场算例
	 ...
	 该算例采用$u=1$，$V=1$的均匀速度场，计算区域为$[0,1]\times[0,1]$ 初始时刻一个边长为$0.2$的正方形中心位于$(0.2,0.2)$，正方形内$z=1$其它区域$z=0$，计算网格为$10\times 10$，计算时间为$t=0.6$。图 \ref{fig:DIP_move}给出计算结果，从中我们看到，在极其稀疏网格下，DIP方法也能准确计算每一个单元点的运动过程。
...
\begin{eqnarray*}
  ...
  \begin{array}{lcl}
	...
	\right.
\end{eqnarray*}
该问题中计算域为$[0.1]\times[0,1]$，采用$N=200\times 200$ 网格。图 \ref{figdisk200}  给出不同时刻的计算结果（$T_1=0$, $T_2=0.25$, $T_3=0.5$, $T_4=0.75$, $T_5=1$ and $T_6=20$），从图中可以看出，DIP方法可以很好保持圆盘的形状。
%
...
我们考虑另一个常见的界面方法测试算例\cite{olsson2005conservative, aulisa2003mixed}，在计算开始，一个半径为0.2的圆圆心位于$(0.5\pi,0.7)$：
\begin{eqnarray*}
  u(x,y)=\left\{
	\begin{array}{lcl}
...
\  \item 多相界面拉伸问题
  ...
  前文指出，现有的界面方法在求解含有三相和三相以上物质的问题时较为困难，这里我们构造一个和上一个算例类似的多相界面算例。


\item 多相界面问题

  前文指出，现有的界面方法在求解含有三相和三相以上物质的问题时较为困难，这里我们构造一个含有多相分界面的算例，计算域为$[-2,2]\times[-2,2]$，初始条件为
 \begin{eqnarray*}
   z_0=\left\{
