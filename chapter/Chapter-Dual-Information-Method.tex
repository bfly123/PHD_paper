...
\begin{eqnarray*}
  ...
  \begin{array}{lcl}
	...
	\right.
\end{eqnarray*}
该问题中计算域为$[0.1]\times[0,1]$，采用$N=200\times 200$ 网格。图 \ref{figdisk200}  给出不同时刻的计算结果（$T_1=0$, $T_2=0.25$, $T_3=0.5$, $T_4=0.75$, $T_5=1$ 和 $T_6=20$），从图中可以看出，即便经过很长时间的旋转，DIP方法仍可以很好保持圆盘的形状。
%
...
图 \ref{fig:DIP_stretch} 中给出了$N=200\times 200$网格下计算结果，分别为$T=\pi$结果和接下来反向运动$\pi$的结果，$T=2\pi$的结果和接下来反向运动$2\pi$的结果，可以看到新方法可以很好的计算界面的拉伸变化，并保持守恒性。
