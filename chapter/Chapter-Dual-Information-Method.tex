...
如果一个单元内有两个甚至多个信息点，如图 \ref{fig:illu_cellpoint_choose}， 根据上面界面位置的规定，首先将$z$较大的单元点消去，如果这之后仍有多个点，在邻居单元中寻找一个$z$值不同的，选择距离$z$值不同单元最近的$p$点作为新的单元点，如果没有不同的，则随意选取。
...
上文中，我们引入了单元信息点，通过单元信息点可以直接求解方程 (\ref{eq:interface_mark}) ，无论是传统的无厚度界面、多于两相物质的多相界面还是具有界面厚度的耗散界面都可以进行模拟。单元信息点只能算是具备了Lagrangian 粒子一半的特性，尽管它弥补了粒子点不能全域覆盖、无法计算耗散界面等缺点，但由于单元点的更新，新的单元点会不断替代旧的单元点，而新的单元点的生成存在误差，导致对界面结构的保持并不像纯粒子方法那么好，这一点将会在算例中展示。为了改进这一不足，我们引入了另外一套全域追踪的信息点，在每一步对单元点进行修正。将这种全域追踪、具有纯粒子点特性的单元点，称为{\bf{粒子信息点}}，简称为粒子点。

\subsection{粒子点的初始化和运动追踪}
...
\begin{equation}
  ip(i)=i, \hspace{0.3cm} X_p(i)=0,\hspace{0.3cm} \bar{z_p}=z(i)
  \end{equation}
...
\subsection{粒子点对单元点的修正过程}
在$t_k$时刻，假设我们已经更新了单元点，如果单元$ip$内含有粒子点，我们采用粒子点代替单元点。关于新的单元点更新策略，无论是对于无厚度界面还是耗散界面来说和前面通过单元点更新单元点是完全相同的，只是用粒子点取代原本的单元点。
